...
<head>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            var activityType="${(publishActivity.type)!}";
            var typeSelect=document.getElementById("type");
            if("school_activity"==activityType){
                typeSelect.options[0].selected=true; 
            }else if("reward_task"==activityType){
            	typeSelect.options[1].selected=true; 
            }else{
            	typeSelect.options[2].selected=true; 
            }
            
            var activityWay="${(publishActivity.way)!}";
            var waySelect=document.getElementById("way");
            if("ask_about"==activityWay){
            	waySelect.options[0].selected=true; 
            }else if("part_time"==activityWay){
            	waySelect.options[1].selected=true; 
            }else{
            	waySelect.options[2].selected=true; 
            }
            
            var payType="${(publishActivity.payway)!}";
            var payWaySelect=document.getElementById("payway");
            if("aa"==payType){
            	payWaySelect.options[0].selected=true; 
            }else if("man_a_woman_free"==payType){
            	payWaySelect.options[1].selected=true; 
            }else if("woman_a_man_free"==payType){
            	payWaySelect.options[2].selected=true; 
            }else{
            	payWaySelect.options[3].selected=true; 
            }
        });
    ...
    <form id="form" name="form" style="+zoom:1;">
        ...
        <table class="table-border" width="100%">
            ...
            <tr>
                <td class="th">人物</td>
                <td class="td">
	                <input type="text" id="person_name" name="person_name" class="input easyui-validatebox" value="${(publishActivity.person_name)!}" validType="maxLength[100]" style="width:300px;"/>
	            	<input type="button" onclick="selectProject('选择人物','${path}/person/basePerson_select.do?callback=selectPerson')" value="选择人物">
	            	<input type="text" id="person_id" name="person_id" style="width:300px;visibility:hidden;" value="${(publishActivity.person_id)!}"/>
                </td>
            </tr>
            <tr>
                <td class="th">费用</td>
