...
<head>
    ...
    <script type="text/javascript">
        ...
        function add() {
            ...
            $("#iframe").attr("src", "${path}/cashPool/transferRecord_add.do");
        }
        ...
        function edit(event, value) {
            ...
            $("#iframe").attr("src","${path}/cashPool/transferRecord_edit.do?id="+value);
        }
        ...
        function del(event,val) {
            ...
            $.messager.confirm("提示信息", "你确认要删除该记录吗？", function(b) {
                if (b) {
                    $.ajaxPost("${path}/cashPool/transferRecord_delete.do",{"id":val},function(result){
                        $.messager.show({
                            title:"消息提醒",
                            msg:"记录删除成功",
                            timeout:5000,
            ...
            if(selections.length==0){
                ...
                $.messager.confirm("系统提示", "你确认要删除所选择的记录吗？", function(b){
                    if(b){
                        ...
                        $.ajaxPost("${path}/cashPool/transferRecord_deleteBatch.do", {"ids":ids}, function(result) {
                            $.messager.show({
                                title:"消息提醒",
                                msg:"记录删除成功",
                                timeout:5000,
        ...
        function view(event,value){
            ...
            $("#iframe").attr("src","${path}/cashPool/transferRecord_view.do?id="+value);
        }
        ...
        function refreshPage(){
            window.location="${path}/cashPool/transferRecord_main.do";
            return false;
        }
        ...
        function searchPage(reset) {
            ...
            transferRecord.refresh = "1"; //刷新记录数参数
            $("#table").datagrid("clearSelections").datagrid("load",transferRecord);
        }
    </script>
</head>
...
<div region="west" split="true" border="true" title="操作面板" style="width:250px;padding:10px;">
    <form id="form" name="form" style="display: none">
        <table class="table">
            ...
            <tr>
                <td class="th">提现人</td>
                <td class="td"><input id="operator" name="operator" type="text" class="input"></td>
            </tr>
            <tr>
                <td class="th">提现银行</td>
                <td class="td"><input id="account_name" name="account_name" type="text" class="input" /></td>
            </tr>
            ...
            <tr>
                <td class="th">提现金额</td>
                <td class="td"><input id="amount" name="amount" type="text" class="input easyui-numberbox" min="0" max="99999999.99" precision="2"/></td>
            </tr>
            <tr>
                <td class="th">提现金额从</td>
                <td class="td"><input id="amount_min" name="amount_min" type="text" class="input easyui-numberbox" min="0" max="99999999.99" precision="2"/></td>
            </tr>
            <tr>
                <td class="th">至</td>
                <td class="td"><input id="amount_max" name="amount_max" type="text" class="input easyui-numberbox" min="0" max="99999999.99" precision="2"/></td>
            </tr>
            <tr>
...
<div region="center" title="当前位置：提现记录管理">
    <table id="table"
           url="${path}/cashPool/transferRecord_page.do"
           border="false"         <#--无边框-->
           fit="true"             <#--自动填充宽度高度-->
