...
<head>
    ...
    <script type="text/javascript">
        ...
        function add() {
            ...
            $("#iframe").attr("src", "${path}/event/mpEvent_add.do");
        }
        ...
        function edit(event, value) {
            ...
            $("#iframe").attr("src","${path}/event/mpEvent_edit.do?id="+value);
        }
        ...
        function del(event,val) {
            ...
            $.messager.confirm("提示信息", "你确认要删除该记录吗？", function(b) {
                if (b) {
                    $.ajaxPost("${path}/event/mpEvent_delete.do",{"id":val},function(result){
                        $.messager.show({
                            title:"消息提醒",
                            msg:"记录删除成功",
                            timeout:5000,
            ...
            if(selections.length==0){
                ...
                $.messager.confirm("系统提示", "你确认要删除所选择的记录吗？", function(b){
                    if(b){
                        ...
                        $.ajaxPost("${path}/event/mpEvent_deleteBatch.do", {"ids":ids}, function(result) {
                            $.messager.show({
                                title:"消息提醒",
                                msg:"记录删除成功",
                                timeout:5000,
        ...
        function view(event,value){
            ...
            $("#iframe").attr("src","${path}/event/mpEvent_view.do?id="+value);
        }
        ...
        function refreshPage(){
            window.location="${path}/event/mpEvent_main.do";
            return false;
        }
        ...
        function searchPage(reset) {
            ...
            mpEvent.refresh = "1"; //刷新记录数参数
            $("#table").datagrid("clearSelections").datagrid("load",mpEvent);
        }
    </script>
</head>
...
<div region="west" split="true" border="true" title="操作面板" style="width:250px;padding:10px;">
    <form id="form" name="form" style="display: none">
        <table class="table">
            ...
            <tr>
                <td class="th">至</td>
                <td class="td"><input id="end_date_end" name="end_date_end" type="text" class="input Wdate" onclick="WdatePicker()"/></td>
            </tr>
            <tr>
                <td class="th">事件类型</td>
                <td class="td">
...
<div region="center" title="当前位置：门派事件管理">
    <table id="table"
           url="${path}/event/mpEvent_page.do"
           border="false"         <#--无边框-->
           fit="true"             <#--自动填充宽度高度-->
